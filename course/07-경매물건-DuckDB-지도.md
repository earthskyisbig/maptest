...
| 검색 | 주소·장소 검색(키워드 → 주소 폴백), 결과 목록, 빨간 핀 팝업 |
| 장소 저장 | 팝업에서 별표 색(6색) 선택 후 저장 → 별 마커, 📌 패널, 메모·색 변경·삭제 |
| 주변 | 지도 클릭 지점 반경 300m~2km, 12개 카테고리 시설 검색·목록 |
| 측정 | 📏 거리 · ⬟ 면적(㎡·평): 마우스를 누른 채 드래그로 그리고 더블클릭으로 마무리(클릭으로 점 추가도 가능) · ◎ 반경: 중심 클릭→이동→클릭 · 🧹 지우기 |
| 팝업 | 폭 210px, 본문 190px 초과 시 세로 스크롤 |

마커 색: 경매 파랑 `#2f7df6`, 관심물건 금색 `#f59e0b`, 저장 장소는 선택한 색의 별.
